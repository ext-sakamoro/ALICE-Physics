...
- **B mechanical audit complete** — Iter 1-6 で 32 module ~452 pub item survey、73 items 格下げ、snapshot 20,201 → 19,956 items (−245)、以降は user decision + v1.0-rc.1 design task:
  - **User decision (v1.0 前必須)**: `solver_tgs*` A/B/C Option (Iter 2 deferred、6 module 60+ items)
  - **v1.0-rc.1 design task**: field visibility of `CachedContactPoint` (Iter 2/3) / `DebugLine`+`DebugPoint` (Iter 6) / `scene_io::PhysicsConfig` (Iter 6) — `#[non_exhaustive]` 化 or accessor-based API
  - **Iter 4b (optional)**: CFD cross-module helpers pub(crate) 化 (`MacGrid`/`Grid3d`/`project_pressure`/`g2p_velocity`/`sample_*` 等)
